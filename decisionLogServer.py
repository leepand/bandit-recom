...
from banditevent.eventnative.bandit_data_model import Bandit_decisions_model,Bandit_rewards_model
...
options = {
    "db_info": category(choices=[64, 128, 256, 512], default=256, description="db engine")
}
@modelcloud.setup(options=options)
def setup(opts):
    print("Setup ran, and the db engine is {}".format(opts["db_info"]))
...
def decision_model(model, decisions):
    ...
    db_file = get_db_file(project_id,remote_path)
    db = model(db_file)
